...
sed -i 's/%%FRONTEND_BIND_IP%%/'"$FRONTEND_BIND_IP"'/g; s/%%FRONTEND_PORT%%/'"$FRONTEND_PORT"'/g; s/%%FRONTEND_PUBLIC_ADDRESS%%/'"$FRONTEND_PUBLIC_ADDRESS"'/g; s/%%AUTH_ADDRESS%%/'"$AUTH_ADDRESS"'/g; s/%%AUTH_PORT%%/'"$AUTH_PORT"'/g; s/%%USE_JSON_DB_MANAGER%%/'"$USE_JSON_DB_MANAGER"'/g; s/%%MAX_BACKUP_NUMBER%%/'"$MAX_BACKUP_NUMBER"'/g; s/%%BACKUP_INTERVAL_MINUTES%%/'"$BACKUP_INTERVAL_MINUTES"'/g; s/%%LOAD_ALL_PROTOTYPES%%/'"$LOAD_ALL_PROTOTYPES"'/g; s/%%USE_EQUIPMENT_SLOT_TABLE_CACHE%%/'"$USE_EQUIPMENT_SLOT_TABLE_CACHE"'/g;' "/usr/share/mhserveremu/Config.ini"
